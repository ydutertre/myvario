...
class MyViewCompetitionTaskMap extends Ui.MapView {

  private var fMinLat as Float = 0.0f;
  private var fMaxLat as Float = 0.0f;
  private var fMinLon as Float = 0.0f;
  private var fMaxLon as Float = 0.0f;
  ...
  function fitTaskBounds() as Void {
    ...
    var aTopLeft = task.fromLocalMeters(self.fRefLat, self.fRefLon, self.fMinX, self.fMaxY);
    var aBottomRight = task.fromLocalMeters(self.fRefLat, self.fRefLon, self.fMaxX, self.fMinY);
    self.fMaxLat = aTopLeft[0] as Float;
    self.fMinLon = aTopLeft[1] as Float;
    self.fMinLat = aBottomRight[0] as Float;
    self.fMaxLon = aBottomRight[1] as Float;
    self.bBoundsValid = true;

    var topLeft = new Pos.Location({:latitude=>self.fMaxLat, :longitude=>self.fMinLon, :format=>:degrees});
    var bottomRight = new Pos.Location({:latitude=>self.fMinLat, :longitude=>self.fMaxLon, :format=>:degrees});
    MapView.setMapVisibleArea(topLeft, bottomRight);
    MapView.setScreenVisibleArea(0, 0, Sys.getDeviceSettings().screenWidth, Sys.getDeviceSettings().screenHeight);
  }
  ...
  function drawTaskOverlay(_oDC as Gfx.Dc) as Void {
    ...
    var iCylinderColor = Gfx.COLOR_YELLOW;
...
class MyViewCompetitionTaskMapDelegate extends Ui.BehaviorDelegate {
